...
const AiInput = ({ noteContent, folderId, noteId }: AiInputProps) => {
  ...
  const sendMessage = async (message: string) => {
    if (!message.trim() || !noteContent) return
    ...
    try {
      ...
      setAiResponse("Une erreur est survenue lors de la communication avec l'IA.")
    } finally {
      setIsLoading(false)
    }
  }
        ...
        <div>
          <div className="bg-grayLight relative w-full rounded-xl p-2.5 flex flex-col gap-2.5">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <button
                type="submit"
                className="hover:opacity-70 transition-all duration-300 h-8 w-8 bg-text rounded-full absolute right-5 bottom-5 flex items-center justify-center"
                disabled={isLoading}
              >
                {isLoading ? (
            ...
            <button
              onClick={handleQuestionsVisible}
              className={`${questionsVisible ? "bg-yellow-50" : ""} absolute left-5 bottom-5 text-grayOpacity font-medium text-sm px-2.5 py-1 rounded-lg transition-all hover:bg-zinc-50`}
              style={{ border: "1px solid var(--gray)" }}
            >
              <span className="italic font-semibold text-[16px] mr-1.5">?</span>Questions fréquentes
            </button>
          </div>
