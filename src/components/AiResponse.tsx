...
const AiResponse = ({ aiResponse, userMessage, onBack, folderId }: AiResponseProps) => {
  ...
  return (
    <>
      ...
      <div id="nav-buttons" className="flex flex-row pt-4 gap-2">
        ...
        >
          {copied ? (
            <svg width="18" height="18" fill="none" viewBox="0 0 16 16">
              <path d="M4 8.5L7 11.5L12 5.5" stroke="#6F6F6F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          ) : (
            <Copy color="#6F6F6F" />
          )}
        ...
        >
          <span className="italic text-grayDark font-semibold text-[16px] mr-1.5">+</span> Nouvelle note
        </button>
        <button className="text-grayOpacity font-medium text-sm px-2.5 py-1 rounded-lg transition-all hover:bg-zinc-50 flex flex-row gap-2" style={{ border: "1px solid var(--gray)" }}>
          <Replace color="#6F6F6F" /> Remplacer
        </button>
        <button
