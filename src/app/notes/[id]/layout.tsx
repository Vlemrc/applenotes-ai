...
export default function NoteLayout({
  ...
  const [note, setNote] = useState(dataNotes.find((n) => n.id === Number.parseInt(resolvedParams.id)))
  const router = useRouter()
  const pathname = usePathname()

  useEffect(() => {
    const currentNote = dataNotes.find((n) => n.id === Number.parseInt(resolvedParams.id))
    if (currentNote) {
      setNote(currentNote)
    } else {
      router.push("/")
